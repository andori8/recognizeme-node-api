...
class Migrator {
  ...
  rollback(config) {
    return _bluebird.default.try(() => {
      this.config = getMergedConfig(config, this.config);
      return migrationListResolver.listAllAndCompleted(this.config, this.knex).tap(value => validateMigrationList(this.config.migrationSource, value)).then(val => this._getLastBatch(val)).then(migrations => {
